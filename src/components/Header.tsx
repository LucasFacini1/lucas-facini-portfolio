      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={toggleMenu}
            className="md:hidden p-2 rounded-lg border border-neon-purple/30 hover:border-neon-purple/60 transition-colors duration-300"
          >
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-dark-800/95 backdrop-blur-md border-t border-neon-purple/20"
          >
              ...
              <div className="flex flex-col space-y-4">
                {navItems.map((item, index) => (
                  <motion.button
                    ...
                    onClick={() => scrollToSection(item.id)}
                    className={`text-left font-inter font-medium text-lg uppercase tracking-wider transition-all duration-300 ${
                      currentSection === item.id
                        ? 'text-neon-purple text-glow'
                        : 'text-white/80 hover:text-neon-purple hover:text-glow'
                    }`}
